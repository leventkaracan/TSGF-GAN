...
from model_focusmap import TSGFGAN
from model_focusmap import MultiscaleDiscriminator as DiscriminatorFusion
...
def main(config):
    ...
    G = TSGFGAN(dgf_r=7, dgf_eps=0.1).cuda()
    D = DiscriminatorFusion(input_nc=3, use_sigmoid=False, ndf=64, norm_layer=nn.BatchNorm2d).cuda()
    ...
    for epoch in range(num_epoch): 
        ...
        for img_A, img_B, foc_map, GT in train_loader:
            ...
            pred_focmap, dgf_focmap, fused_img = G(img_A, img_B)
            ...
            real_vid_feat = D(GT)
            fake_vid_feat = D(fused_img.detach())
            ...
            if total_iteration%1 == 0:
                ...
                # generator l1 loss
                img_loss = l1_loss(dgf_focmap, foc_map)
                ffl_loss = ffl(fused_img, GT)
                gen_loss += 1.0 * img_loss
                gen_loss += 1.0 * ffl_loss
            ...
            if(total_iteration%1==0):
                print('Epoch [%d/%d], Iter [%d/%d], gen_loss: %.4f, dis_real_loss: %.4f,  dis_fake_loss: %.4f, img_loss: %.4f, ffl_loss: %.7f'% (epoch, num_epoch, it, len(train_loader), avg_gen_loss/(itg), avg_dis_real_loss/it, avg_dis_fake_loss/it, avg_img_loss/(itg), avg_ffl_loss/(itg)))
