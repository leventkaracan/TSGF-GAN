...
def main(config):
    ...
    elif(config.test_dataset == "MFI-WHU"):
      test_data = MFIWHU(img_root, transform)
        
    test_loader = data.DataLoader(test_data, batch_size=1, shuffle=False, num_workers=config.num_workers)
    G = TSGFGAN(dgf_r=7, dgf_eps=0.5).cuda()
    ...
    for img_A, img_B in test_loader:
        with torch.no_grad():
            ...
            focus_map_dgf = G(img_A, img_B)[1]


            fusion_used_time = time.time() - fusion_start_time
            print(fusion_used_time)
            save_image(focus_map_dgf, os.path.join(output_root, "%d_pred.jpg"%no))
            focus_map = threshold(focus_map_dgf, t=0.5).float()
            ...
            save_image((focus_map), os.path.join(output_root,"%d_thresh.png"%no))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='tsgf-gan-mfif_best')
